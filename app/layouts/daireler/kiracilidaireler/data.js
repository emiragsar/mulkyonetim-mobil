...
// Custom hook for Kiracilidaireler data
export const useKiracilidairelerData = () => {
  ...
  const fetchDaireler = useCallback(async () => {
    try {
      ...
      // Filter daires that have current tenants
      const dairesWithTenants = daireRes.data.filter(
        (item) =>
          item.Guncel_Kiraci_ID !== null && item.Guncel_Kiraci_ID !== undefined
      );

      const enrichedRows = dairesWithTenants.map((item) =>
...
export default useKiracilidairelerData;
